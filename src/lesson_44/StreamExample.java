...
public class StreamExample {
    ...
    public static void main(String[] args) {
        ...
        ---
        distinct() - удаляет дубликаты из потока. Сравниваем (определяем равенство) методом equals().

        ---
        void peek(Consumer<T> action) - выполняет действие для каждого элемента потока.

        ---
        limit (long maxSize) - ограничивает количество элементов потока заданным значением. В потоке может быть не больше maxSize.
        ...
        ---
        boxed() - используется для преобразования потока примитивов (IntStream, DoubleStream) в поток соответствующих им
        обёрток (Stream<Integer>, Stream<Double)
         */
...
//                .mapToObj(i -> Integer.valueOf(i))
    ...
    private static void task12() {
        ...
        // Получить список из трёх самых маленьких чисел из списка
        ...
        // Получить массив чисел, отбросив два самых маленьких
    ...
    private static void task11() {
        ...
        Optional<String> longestName = cats.stream()
                ...
                .map(Cat::getName)
                .filter(Objects::nonNull) // проверка имён null
...
//                .max((name1, name2) -> name1.length() - name2.length());
    ...
    private static void task10() {
        ...
        // min, max, Optional
        // Optional<T> - класс объектов, который может содержать или объект тип T или null
        // Optional<User>

        List<Integer> integers = List.of(5, 4, 14, 59, 32, 24, -1, -6);

        // Необходимо найти максимальное отрицательное число
        ...
        System.out.println(max.isPresent()); // вернёт true, если значение присутствует (там не null)
        System.out.println(max.isEmpty()); // вернёт true, если значение отсутствует (там внутри null) // Java 11

        if (max.isPresent()) {
            Integer value = max.get(); // возвращает значение, если оно присутствует. Если внутри null, будет исключение NoSuchElementException (ошибка)
            System.out.println("value: " + value);
        } else {
            System.out.println("Завёрнут null");
        }

        // Возвращает значение, если оно присутствует. Если внутри null - вернёт defaultValue
        ...
        Optional.of(new Object()); // возвращает Optional с не-null значением. Если попытаться завернуть null 0, будет NPE.
        Optional.ofNullable(null); // возвращает Optional. Можно завернуть или значение, или null (вернёт пустой Optional)
    }
    ...
    private static Cat findCat3(String name) {
        List<Cat> cats = getListCats();
    ...
    private static void task9() {
        ...
        // Cоздание стрима из элементов EntrySet
        map.entrySet().stream()
    ...
    private static void task8() {
        ...
        // Мультикурсор Windows: Alt + Shift + click | Mac: Option + Shift + click
...
//                .filter(c -> Objects.nonNull(c))          // оставить только не null
    ...
    private static void task7() {
        // Изменить имя кошек, вес которых меньше 5
...
//    catStream.forEach(System.out::println);
    ...
    private static void task6() {
        ...
        /*
        Эффективность: имена извлекаются у всех объектов Cat, даже у тех, которые будут позже отфильтрованы.
         */
